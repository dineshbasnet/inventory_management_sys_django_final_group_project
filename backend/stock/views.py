...
from .serializers import (StockSerializer, StockMovementSerializer,
                           StockAddRemoveSerializer, StockAdjustSerializer)
...
class StockViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def add(self, request):
        ...
        stock = StockService.add_stock(
            product=product, quantity=d['quantity'],
    ...
    def adjust(self, request):
        ...
        stock = StockService.adjust_stock(
            product=product, new_quantity=d['new_quantity'],
            notes=d.get('notes', ''), user=request.user,
        )
        return Response(StockSerializer(stock).data)
